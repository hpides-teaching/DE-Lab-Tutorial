...
void PerfWrapper::initialize()
{
    ...
    if (default_counters.size() > config.max_counters_per_group() * config.max_groups())
    {
        throw std::runtime_error(
            "Too many perf counters have been defined");
    }
...
void PerfWrapper::start()
{
    if (!event_counter.has_value())
    {
        throw std::runtime_error("PerfWrapper::start() has been called before initialize() has been called.");
    }
    try
    {
...
void PerfWrapper::stop()
{
    if (!event_counter.has_value())
    {
        throw std::runtime_error("PerfWrapper::stop() has been called before initialize() has been called.");
    }
...
perf::CounterResult PerfWrapper::result(std::uint64_t normalization)
{
    if (!event_counter.has_value())
    {
        throw std::runtime_error("PerfWrapper::result() has been called before initialize() has been called.");
    }
